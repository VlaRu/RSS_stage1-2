...
const garageUrl: string = `${url}/garage`;
...
export const createCar = async (body: HTMLBodyElement) => (await fetch(garageUrl, {
  method: 'POST',
...
export const updateCar = async (id: number, body: HTMLBodyElement) => (await fetch(`${garageUrl}/${id}`, {
  method: 'PUT',
  body: JSON.stringify(body),
  headers: {
    'Content-Type': 'application/json'
