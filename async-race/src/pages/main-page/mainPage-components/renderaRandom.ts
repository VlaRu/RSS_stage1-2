...
const models: string[] = ['Acura', 'Alfa Romeo', 'Alpine', 'Apollo', 'Apple', 'Aston Martin', 'Audi', 'Automobili Pininfarina', 'Bentley', 'BMW', 'Bollinger',
...
const names: string[] = ['Durango', 'Ram', 'Challenger', 'Charger', 'Grand Caravan', 'X7', 'X5', 'X3', 'X6 M', 'X6', 'X1', 'X4', 'C3 Aircross', 'C5 Aircross', 'Duster', 'CR-V', 'Corolla',
...
export const generateRandomCars = (count: number = 100) => new Array(count).fill(1).map(() => ({ name: getRandomName(), color: getRandomColor()}));

export const getRundomCars = {
  text: 'Get 100 car',
  class: 'random-car_button',
  onClick: async () => {
    const carsRender = generateRandomCars();
    carsRender.forEach(async car => {
      try {
        await createCar(car);
        resetData();
        console.log(`Car updated: ${car.name}`);
      } catch (error) {
        console.error(`Error updating car ${car.name}: ${error}`);
      }
    });
  },
};